# Microsoft Number Checker API — Bulk Verification | NumberChecker

The **Microsoft Number Checker API** accepts a text file of phone numbers, creates an asynchronous bulk task, and exports a result file. This repository contains a runnable integration in seven languages.

- **Product page:** https://numberchecker.ai/products/microsoft-checker
- **API documentation:** https://docs.numberchecker.ai/microsoft-checker
- **API base URL:** `https://api.numberchecker.ai`
- **Authentication:** `X-API-Key`
- **Get an API key:** https://platform.numberchecker.ai

> [!NOTE]
> This repository covers the standalone product code `microsoft` only. Other product codes are outside its scope.

## Product facts

| API fact | Value |
|---|---|
| Product code | `microsoft` |
...
1. Submit the input file to `POST /v1/tasks` with `task_type=microsoft`.
...
Only `microsoft`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/microsoft-checker
- **Documentation:** https://docs.numberchecker.ai/microsoft-checker
